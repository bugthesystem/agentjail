  ...
  return (
    <div className="px-5 py-2 flex items-center justify-between border-t border-ink-800 text-[11px] mono text-ink-500">
      ...
      <div className="flex items-center gap-1">
        <Btn onClick={() => onGo(1)}         disabled={page <= 1}>« first</Btn>
        <Btn onClick={() => onGo(page - 1)}  disabled={page <= 1}>[</Btn>
        <span className="px-3 text-ink-300">{page} / {pageCount}</span>
        <Btn onClick={() => onGo(page + 1)}  disabled={page >= pageCount}>]</Btn>
        <Btn onClick={() => onGo(pageCount)} disabled={page >= pageCount}>last »</Btn>
      </div>
    </div>
  );
}
